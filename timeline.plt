...
boxc_grey   = 180
boxc_fac    = 0.2
boxcolor(c) = ((boxc_grey + int(((int(c) & 0xff0000) >> 16)*boxc_fac))<<16) \
            + ((boxc_grey + int(((int(c) & 0x00ff00) >>  8)*boxc_fac))<< 8) \
            + ( boxc_grey + int(((int(c) & 0x0000ff)       *boxc_fac)))

plot \
	fn u (0):3:(start_x):(end_x):($3+box_hi):($3+box_low):(boxcolor($8)) w boxxy lc rgb var,\
	fn u (0):1:(0):($2-$1):8 w vector nohead lc rgb var lw 8,\
	fn u (0):3:(boxcolor($8)) w p pt 7 lc rgb var ps 1.5,\
